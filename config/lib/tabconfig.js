...
;(function () {
  ...
  const forceFallback =
    navigator.userAgent.indexOf('QtWebEngine') !== -1 || 'obsstudio' in window
  ...
  class Tabconfig {
    constructor() {
      ...
      this.selections = COLUMN_SORTABLE.map((_) => {
        ...
        return `<option value="${_}" data-locale="col.${
          k.split('.')[0]
        }._+: +col.${k}.1"></option>`
      }).join('')
    ...
    append(o) {
      let tab = document.importNode(this.template.tab, true).firstElementChild
      let pane = document.importNode(this.template.pane, true).firstElementChild
      ...
      if (MIGRATE_SORTABLE[o.sort]) sort.value = MIGRATE_SORTABLE[o.sort]
      else sort.value = o.sort
      ...
      if (window.locale.loaded) window.locale.localizeAll(pane)
