...
window.$ = function $(root, selector, index) {
  ...
  if (/^#[0-9a-z_\-]+?$/.test(selector))
    ...
    if (index === 0) return root.querySelector(selector)
    else return root.querySelectorAll(selector)[index]
  else return root.querySelectorAll(selector)
}

window.$map = function $map(root, selector, callback) {
  let _$
...
window.parseSvg = function parseSvg(text) {
  let parser = new DOMParser()
  return parser.parseFromString(text, 'image/svg+xml')
}

window.updateObject = function updateObject(obj /*, ... */) {
  for (let i = 1; i < arguments.length; i++) {
    for (let prop in arguments[i]) {
      let val = arguments[i][prop]
      if (typeof val == 'object') updateObject(obj[prop], val)
      else obj[prop] = val
    }
  }
  return obj
...
window.resolveDotIndex = function resolveDotIndex(o, p, v) {
  // Example:
  //   o: {a: {b: {c: 1}}}
  //   p: 'a.b.c'
  //   returns: 1
  if (typeof p === 'string') return resolveDotIndex(o, p.split('.'), v)
  else if (p.length === 0) return o
  else if (p[0] != null && !(p[0] in o)) return undefined
  else if (p.length === 1 && v !== undefined) return (o[p[0]] = v)
  else return resolveDotIndex(o[p[0]], p.slice(1), v)
}

window.resolveClass = function resolveJobFromName(_job, _name) {
  _job = _job || ''
...
window.resolveOwner = function resolveOwner(_) {
  let o = /^.+? \((.+?)\)$/.exec(_)
  return (o && o[1]) || undefined
}

window.animateNumber = function animateNumber(element, to, option) {
...
window.isYou = function isYou(name, list) {
...
window.pFloat = function parseLocaledFloat(string) {
  if (typeof string !== 'string') return string
  else return parseFloat(string.replace(',', '.'))
}

window.pInt = function parseLocaledInteger(string) {
  if (typeof string !== 'string') return string
  else return parseInt(string.replace(/[,.]/g, ''))
}

window.ABBREVIATION_SUFFIXES = ['', 'k', 'M', 'G', 'T']

window.formatDps = function formatDPSNumberWithSmalls(
  number,
  opt,
  type,
  forceInt,
  ...
  const abbrUnit = abbr
    ? Math.max(0, Math.min(4, Math.floor((exponential - 1) / 3)))
    : 0
  ...
  const upperEndsAt = Math.max(
    0,
    number.length - decimalLength - (abbrUnit ? 0 : 3),
  )
...
window.sanitize = (_) => _.toLowerCase().replace(/[^a-zA-Z0-9]/g, '-')
