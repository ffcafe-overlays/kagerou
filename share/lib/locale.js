...
;(function () {
  ...
  class Locale {
    ...
    _skill(n) {
      ...
      if (!o) return []
      else if (o[2] === undefined) return [n, false]
      else if (o[2] === ' (*)') return [o[1], true]
      else return []
    }
    ...
    localizeAll(under, _stack) {
      // retry stack
      if (
        !(this.current in this.L) ||
      ...
      $map(under, '[data-locale]', (_) => {
        ...
        c.split('|').forEach((token) => {
          ...
          if (key) _.setAttribute(key, value)
          else _.innerHTML = value
        })
      })
    }
